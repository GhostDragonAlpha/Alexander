...
// Irrigation system - automated watering
UCLASS()
class ALEXANDER_API AIrrigationSystem : public AActor
{
	GENERATED_BODY()
...
public:
	...
	void InitializeSystem(float WaterCapacity, float FlowRate, float CoverageRadius);

	// Add farm plot to irrigation coverage
	UFUNCTION(BlueprintCallable, Category = "Irrigation")
	...
	// Water all covered plots
	UFUNCTION(BlueprintCallable, Category = "Irrigation")
	void WaterAllCoveredPlots();

	// Set watering schedule
	UFUNCTION(BlueprintCallable, Category = "Irrigation")
	void SetWateringSchedule(float Interval, float Duration);

	// Refill water tank
	UFUNCTION(BlueprintCallable, Category = "Irrigation")
	void RefillWaterTank(float Amount);

	// Get system status
	UFUNCTION(BlueprintCallable, Category = "Irrigation")
	bool IsSystemOperational() const { return bIsOperational; }
...
private:
	...
	void CheckWateringSchedule(float DeltaTime);
